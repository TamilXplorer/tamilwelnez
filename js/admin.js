(function () {
        ...
        try {
            ...
            if (dbOrders && !ordErr) {
                const mappedOrders = dbOrders.map(o => ({
                    ...
                    customerPhone: o.customer_phone || o.customerPhone || "N/A",
                    ...
                    razorpayPaymentId: o.razorpay_payment_id || o.razorpayPaymentId || null,
                    pincode: o.pincode,
                    status: o.status
                }));
            ...
            if (dbSettings && !setErr) {
                dbSettings.forEach(s => {
                    ...
                    } else if (s.key === 'wellnez_razorpay') {
                        localStorage.setItem("wellnez_razorpay", JSON.stringify(s.value));
                    } else if (s.key === 'wellnez_announcement_ribbon') {
                        localStorage.setItem("wellnez_announcement_ribbon", JSON.stringify(s.value));
                    }
                });
            }

            // 5. Sync Users
            const { data: dbUsers, error: usrErr } = await db.from('users').select('*');
            if (dbUsers && !usrErr && dbUsers.length > 0) {
                ...
                if (!mappedUsers.some(u => u.role === "admin")) {
                    mappedUsers.push({ username: "admin", email: "admin@wellnez.com", password: "password123", role: "admin" });
                }
                localStorage.setItem("wellnez_users", JSON.stringify(mappedUsers));
            }

            // 6. Sync Wishlists
            const { data: dbWishlists, error: wishErr } = await db.from('wishlists').select('*');
            if (dbWishlists && !wishErr) {
                dbWishlists.forEach(w => {
                    localStorage.setItem(`wellnez_wishlist_${w.email}`, JSON.stringify(w.product_ids || []));
                });
            }
        } catch (err) {
    ...
    const Store = {
        ...
        getPaymentMethods() {
            const defaults = [
                { id: "Razorpay", name: "Razorpay (Cards, UPI, NetBanking, Wallets)", icon: "fa-solid fa-credit-card", enabled: true },
                { id: "UPI", name: "UPI Pay (GPay / PhonePe / Paytm)", icon: "fa-solid fa-qrcode", enabled: true },
                { id: "Card", name: "Credit/Debit Card", icon: "fa-regular fa-credit-card", enabled: true },
                { id: "COD", name: "Cash on Delivery (COD)", icon: "fa-solid fa-hand-holding-dollar", enabled: true }
            ];
            const stored = localStorage.getItem("wellnez_payments") || localStorage.getItem("wellnez_payment_methods");
            if (!stored) {
                localStorage.setItem("wellnez_payments", JSON.stringify(defaults));
                return defaults;
            }
            let parsed = JSON.parse(stored);
            if (!parsed.some(m => m.id === "Razorpay" || m.id === "razorpay")) {
                parsed.unshift({ id: "Razorpay", name: "Razorpay (Cards, UPI, NetBanking, Wallets)", icon: "fa-solid fa-credit-card", enabled: true });
                localStorage.setItem("wellnez_payments", JSON.stringify(parsed));
            }
            return parsed;
        },
        togglePaymentMethod(id) {
            const methods = this.getPaymentMethods();
            const m = methods.find(x => x.id === id || x.id.toLowerCase() === id.toLowerCase());
            if (m) {
                m.enabled = !m.enabled;
        ...
        getRazorpaySettings() {
            const stored = localStorage.getItem("wellnez_razorpay");
            return stored ? JSON.parse(stored) : { keyId: "rzp_live_TEwsMlHmlKcwe9", keySecret: "qEd0350qLJafTlfH456BiHAz", enabled: true, mode: "live" };
        },
        saveRazorpaySettings(keyId, keySecret, enabled, mode) {
            const settingsData = { keyId, keySecret, enabled, mode };
            localStorage.setItem("wellnez_razorpay", JSON.stringify(settingsData));

            if (db) {
                db.from('settings').upsert({ key: 'wellnez_razorpay', value: settingsData }).then(({ error }) => {
                    if (error) console.error("Error saving Razorpay settings to Supabase:", error);
                });
            }
        },
        getAnnouncementRibbon() {
            const defaults = {
                enabled: true,
                text: "Sweat-Proof, Food-Proof, Bite-Proof, Pillow-Proof, Dermatologically Tested, 100% Vegan & Cruelty-Free",
                bgColor: "#000000",
                textColor: "#FFFFFF",
                speed: 25
            };
            return JSON.parse(localStorage.getItem("wellnez_announcement_ribbon")) || defaults;
        },
        saveAnnouncementRibbon(data) {
            localStorage.setItem("wellnez_announcement_ribbon", JSON.stringify(data));
            if (db) {
                db.from('settings').upsert({ key: 'wellnez_announcement_ribbon', value: data }).then(({ error }) => {
                    if (error) console.error("Error saving ribbon settings to Supabase:", error);
                });
            }
        },
        getCMSContent() {
            const defaults = {
                heroBadge: "BARRIER-FIRST SKINCARE ERA",
                heroTitle: "SKIN BARRIER FIRST. ACTIVES SECOND.",
                heroSubtitle: "Dermatologist-formulated daily skincare designed to defend, restore, and lock in your natural moisture barrier.",
                heroImage: "images/hero.png",
                whyTitle: "Why Choose your Wellnez?",
                whyImage: "https://images.unsplash.com/photo-1522335789203-aabd1fc54bc9?auto=format&fit=crop&q=80&w=600",
                aboutTitle: "Our Story",
                aboutPara1: "Welcome to your Wellnez. We believe in one simple, powerful skincare philosophy: Skin Barrier First. Actives Second.",
                aboutPara2: "Founded in 2026, our mission was born out of frustration with over-complicated skincare routines that relied on harsh active chemicals. We engineer daily skincare basics designed to defend, restore, and lock in your natural skin lipids.",
                aboutQuote: "\"Your skin barrier is your body's shield. If it is damaged, no active serum will give you that coveted glass skin glow. We focus on rebuilding your barrier first.\"",
                aboutCrueltyText: "All our products are dermatologist-tested, pH-balanced (5.5 - 6.0), vegan, and 100% cruelty-free.",
                aboutImage: "https://images.unsplash.com/photo-1556228720-195a672e8a03?auto=format&fit=crop&q=80&w=800"
            };
            const saved = JSON.parse(localStorage.getItem("wellnez_cms_content"));
            return saved ? { ...defaults, ...saved } : defaults;
        },
        saveCMSContent(data) {
            localStorage.setItem("wellnez_cms_content", JSON.stringify(data));
            if (db) {
                db.from('settings').upsert({ key: 'wellnez_cms_content', value: data }).then(({ error }) => {
                    if (error) console.error("Error saving CMS content to Supabase:", error);
                });
            }
        },
        getAdminKPIs() {
        ...
        getCustomers() {
            const orders = this.getOrders();
            const users = JSON.parse(localStorage.getItem("wellnez_users")) || [];
            const customersMap = {};

            // Populate registered users
            users.filter(u => u.role !== "admin").forEach(u => {
                const likedList = JSON.parse(localStorage.getItem(`wellnez_wishlist_${u.email}`)) || [];
                customersMap[u.email] = {
                    ...
                    ordersCount: 0,
                    orders: [],
                    likedProductIds: likedList
                };
            });

            // Populate orders & guest buyers
            orders.forEach(o => {
                const email = o.customerEmail || "walkin@wellnez.com";
                if (!customersMap[email]) {
                    const likedList = JSON.parse(localStorage.getItem(`wellnez_wishlist_${email}`)) || [];
                    customersMap[email] = {
                ...
                customersMap[email].orders.push(o);
                if (o.customerName && (!customersMap[email].name || customersMap[email].name === email.split('@')[0])) {
                    customersMap[email].name = o.customerName;
                }
                if (o.phone && customersMap[email].phone === "N/A") {
                    customersMap[email].phone = o.phone;
                }
            });
    ...
    const renderPaymentsTab = () => {
        ...
        const rzp = Store.getRazorpaySettings();
        const ribbon = Store.getAnnouncementRibbon();
        return `
            <!-- Announcement Ribbon Banner Settings Card -->
            <div class="admin-section-box" style="background:#FFF; padding:25px; border:1px solid var(--border-color); border-radius:8px; margin-bottom:25px;">
                <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid var(--border-color); padding-bottom:15px; margin-bottom:25px;">
                    <div style="display:flex; align-items:center; gap:12px;">
                        <div style="width:40px; height:40px; border-radius:8px; background:#000000; color:#FFFFFF; display:flex; align-items:center; justify-content:center; font-size:1.2rem;"><i class="fa-solid fa-scroll"></i></div>
                        <div>
                            <h3 style="border:none; margin:0;">Homepage Scrolling Announcement Ribbon (Marquee)</h3>
                            <p style="color:var(--text-muted); font-size:0.8rem; margin:0;">Configure right-to-left scrolling banner text, speed & colors on homepage</p>
                        </div>
                    </div>
                    <span style="font-size:0.75rem; font-weight:700; background:${ribbon.enabled ? '#e8f8f0' : '#ffebee'}; color:${ribbon.enabled ? '#27ae60' : '#e74c3c'}; padding:6px 12px; border-radius:20px;">
                        <i class="fa-solid fa-circle"></i> ${ribbon.enabled ? 'Ribbon Active' : 'Ribbon Hidden'}
                    </span>
                </div>

                <form id="ribbon-settings-form" style="display:flex; flex-direction:column; gap:18px;">
                    <div class="form-group">
                        <label for="ribbon-text-input" style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">Announcement Text (Comma Separated)</label>
                        <textarea id="ribbon-text-input" class="form-control" rows="2" placeholder="Sweat-Proof, Food-Proof, Bite-Proof, Pillow-Proof, 100% Vegan" required style="width:100%; padding:10px; border:1px solid var(--border-color); border-radius:4px; font-weight:600;">${ribbon.text || ''}</textarea>
                        <p style="font-size:0.75rem; color:var(--text-muted); margin-top:5px;">Separate phrases with commas. They will continuously scroll right-to-left across the homepage ribbon strip.</p>
                    </div>

                    <div style="display:grid; grid-template-columns: repeat(4, 1fr); gap:15px;">
                        <div class="form-group">
                            <label for="ribbon-enabled-select" style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">Ribbon Status</label>
                            <select id="ribbon-enabled-select" class="form-control" style="width:100%; padding:10px; border:1px solid var(--border-color); border-radius:4px;">
                                <option value="true" ${ribbon.enabled ? 'selected' : ''}>Show Ribbon</option>
                                <option value="false" ${!ribbon.enabled ? 'selected' : ''}>Hide Ribbon</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="ribbon-bg-color" style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">Background Color</label>
                            <input type="color" id="ribbon-bg-color" value="${ribbon.bgColor || '#000000'}" style="width:100%; height:40px; padding:2px; border:1px solid var(--border-color); border-radius:4px; cursor:pointer;">
                        </div>
                        <div class="form-group">
                            <label for="ribbon-text-color" style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">Text Color</label>
                            <input type="color" id="ribbon-text-color" value="${ribbon.textColor || '#FFFFFF'}" style="width:100%; height:40px; padding:2px; border:1px solid var(--border-color); border-radius:4px; cursor:pointer;">
                        </div>
                        <div class="form-group">
                            <label for="ribbon-speed-select" style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">Scroll Speed</label>
                            <select id="ribbon-speed-select" class="form-control" style="width:100%; padding:10px; border:1px solid var(--border-color); border-radius:4px;">
                                <option value="15" ${ribbon.speed == 15 ? 'selected' : ''}>Fast (15s)</option>
                                <option value="25" ${ribbon.speed == 25 || !ribbon.speed ? 'selected' : ''}>Medium (25s)</option>
                                <option value="35" ${ribbon.speed == 35 ? 'selected' : ''}>Slow (35s)</option>
                            </select>
                        </div>
                    </div>

                    <button type="submit" class="btn btn-chrome" style="height:45px; background:#000000; color:#FFFFFF; border:none; border-radius:4px; font-weight:700; cursor:pointer; width:fit-content; padding:0 25px;">
                        <i class="fa-solid fa-floppy-disk" style="margin-right:8px;"></i> Save Ribbon Settings
                    </button>
                </form>
            </div>
            ...
            <div class="admin-section-box" style="background:#FFF; padding:25px; border:1px solid var(--border-color); border-radius:8px; margin-bottom:25px;">
                <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid var(--border-color); padding-bottom:15px; margin-bottom:25px;">
                    <div style="display:flex; align-items:center; gap:12px;">
                        <div style="width:40px; height:40px; border-radius:8px; background:#0c2340; color:#00baf2; display:flex; align-items:center; justify-content:center; font-size:1.3rem; font-weight:800;">R</div>
                        <div>
                            <h3 style="border:none; margin:0;">Razorpay Payment Gateway API</h3>
                            <p style="color:var(--text-muted); font-size:0.8rem; margin:0;">Configure Razorpay Live / Test API Keys for instant checkout payments</p>
                        </div>
                    </div>
                    <span style="font-size:0.75rem; font-weight:700; background:${rzp.enabled ? '#e8f8f0' : '#ffebee'}; color:${rzp.enabled ? '#27ae60' : '#e74c3c'}; padding:6px 12px; border-radius:20px;">
                        <i class="fa-solid fa-circle"></i> ${rzp.enabled ? (rzp.mode === 'live' ? 'Razorpay Live Gateway Active' : 'Razorpay Test Gateway Active') : 'Gateway Disabled'}
                    </span>
                </div>

                <form id="razorpay-settings-form" style="display:flex; flex-direction:column; gap:15px; max-width:650px;">
                    <div class="grid-2" style="display:grid; grid-template-columns:1fr 1fr; gap:20px;">
                        <div class="form-group">
                            <label for="rzp-key-id" style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">Razorpay Key ID</label>
                            <input type="text" id="rzp-key-id" class="form-control" placeholder="rzp_live_..." value="${rzp.keyId || ''}" required style="width:100%; padding:10px; border:1px solid var(--border-color); border-radius:4px; font-family:monospace;">
                        </div>
                        <div class="form-group">
                            <label for="rzp-key-secret" style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">Razorpay Key Secret</label>
                            <input type="password" id="rzp-key-secret" class="form-control" placeholder="••••••••••••••••" value="${rzp.keySecret || ''}" style="width:100%; padding:10px; border:1px solid var(--border-color); border-radius:4px; font-family:monospace;">
                        </div>
                    </div>
                    <div class="grid-2" style="display:grid; grid-template-columns:1fr 1fr; gap:20px;">
                        <div class="form-group">
                            <label for="rzp-mode" style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">Environment Mode</label>
                            <select id="rzp-mode" class="form-control" style="width:100%; padding:10px; border:1px solid var(--border-color); border-radius:4px;">
                                <option value="live" ${rzp.mode === 'live' ? 'selected' : ''}>Live Production Mode</option>
                                <option value="test" ${rzp.mode === 'test' ? 'selected' : ''}>Test Sandbox Mode</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="rzp-enabled" style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">Gateway Status</label>
                            <select id="rzp-enabled" class="form-control" style="width:100%; padding:10px; border:1px solid var(--border-color); border-radius:4px;">
                                <option value="true" ${rzp.enabled ? 'selected' : ''}>Enabled for Customers</option>
                                <option value="false" ${!rzp.enabled ? 'selected' : ''}>Disabled</option>
                            </select>
                        </div>
                    </div>
                    <button type="submit" class="btn btn-chrome" style="height:45px; background:#0c2340; color:#FFFFFF; border:none; border-radius:4px; font-weight:700; cursor:pointer; width:fit-content; padding:0 25px;">
                        <i class="fa-solid fa-floppy-disk" style="margin-right:8px;"></i> Save Razorpay Gateway Credentials
                    </button>
                </form>
            </div>
    ...
    const renderCustomersTab = () => {
        ...
        const customerRows = customers.map(c => `
            <tr style="border-bottom:1px solid var(--border-color);">
                ...
                <td style="padding:12px; font-weight:700; color:#27ae60;">₹${c.totalSpent}</td>
                <td style="padding:12px;"><span style="background:#eef2f7; padding:4px 8px; border-radius:12px; font-weight:700; font-size:0.78rem;">${c.ordersCount} orders</span></td>
                <td style="padding:12px;"><span style="color:#e74c3c; font-weight:700;"><i class="fa-solid fa-heart"></i> ${c.likedProductIds.length} liked</span></td>
                <td style="padding:12px;">
                    <button class="btn btn-outline view-customer-details-btn" data-email="${c.email}" style="padding:6px 12px; font-size:0.75rem; border-color:var(--border-dark);">
        ...
        return `
            <div class="admin-section-box" style="background:#FFF; padding:25px; border:1px solid var(--border-color); border-radius:8px;">
                <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid var(--border-color); padding-bottom:15px; margin-bottom:20px;">
                    <h3 style="border:none; margin:0;">Customer Profiles & Activity</h3>
                    <span style="font-size:0.8rem; color:var(--text-muted); font-family:var(--font-heading);">Total Registered & Active Buyers: <strong>${customers.length}</strong></span>
                </div>
                <div class="table-responsive" style="overflow-x:auto;">
                    <table class="admin-table" style="width:100%; border-collapse:collapse; text-align:left;">
                        <thead>
                            <tr style="background:#FAFAFA; border-bottom:1px solid var(--border-color); font-size:0.8rem; text-transform:uppercase;">
                                ...
                                <th style="padding:12px;">Liked Products</th>
                                <th style="padding:12px;">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${customerRows ? customerRows : '<tr><td colspan="7" style="text-align:center; padding:30px;">No customers registered yet.</td></tr>'}
                        </tbody>
                    </table>
                </div>
            </div>
    ...
    const renderCMSTab = () => {
        const cms = Store.getCMSContent();

        return `
            <div class="admin-section-box" style="background:#FFF; padding:25px; border:1px solid var(--border-color); border-radius:8px; margin-bottom:25px;">
                <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid var(--border-color); padding-bottom:15px; margin-bottom:25px;">
                    <div style="display:flex; align-items:center; gap:12px;">
                        <div style="width:40px; height:40px; border-radius:8px; background:#000000; color:#FFFFFF; display:flex; align-items:center; justify-content:center; font-size:1.2rem;"><i class="fa-solid fa-pen-to-square"></i></div>
                        <div>
                            <h3 style="border:none; margin:0;">Homepage & About Page Content CMS</h3>
                            <p style="color:var(--text-muted); font-size:0.8rem; margin:0;">Edit titles, subtitles, quotes, paragraphs & images across storefront pages</p>
                        </div>
                    </div>
                    <span style="font-size:0.75rem; font-weight:700; background:#e8f8f0; color:#27ae60; padding:6px 12px; border-radius:20px;">
                        <i class="fa-solid fa-cloud-arrow-up"></i> Live Cloud CMS
                    </span>
                </div>

                <form id="cms-settings-form" style="display:flex; flex-direction:column; gap:25px;">
                    <!-- Homepage Section CMS -->
                    <div style="border:1px solid var(--border-color); border-radius:6px; padding:20px; background:#FAFAFA;">
                        <h4 style="font-family:var(--font-heading); text-transform:uppercase; font-size:0.9rem; margin-bottom:15px; color:#000; display:flex; align-items:center; gap:8px;">
                            <i class="fa-solid fa-house" style="color:var(--text-muted);"></i> 1. Homepage Content & Assets
                        </h4>

                        <div style="display:flex; flex-direction:column; gap:15px;">
                            <div class="form-group">
                                <label for="cms-hero-badge" style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">Hero Badge Text</label>
                                <input type="text" id="cms-hero-badge" class="form-control" value="${cms.heroBadge || ''}" required style="width:100%; padding:10px; border:1px solid var(--border-color); border-radius:4px; font-weight:600;">
                            </div>

                            <div class="form-group">
                                <label for="cms-hero-title" style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">Hero Main Title</label>
                                <input type="text" id="cms-hero-title" class="form-control" value="${cms.heroTitle || ''}" required style="width:100%; padding:10px; border:1px solid var(--border-color); border-radius:4px; font-weight:700; font-size:1.1rem;">
                            </div>

                            <div class="form-group">
                                <label for="cms-hero-subtitle" style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">Hero Subtitle / Description</label>
                                <textarea id="cms-hero-subtitle" class="form-control" rows="2" required style="width:100%; padding:10px; border:1px solid var(--border-color); border-radius:4px;">${cms.heroSubtitle || ''}</textarea>
                            </div>

                            <!-- Hero Image File/URL -->
                            <div class="form-group">
                                <label style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">Homepage Hero Image</label>
                                <div style="display:flex; gap:15px; align-items:center;">
                                    <img id="cms-hero-img-preview" src="${cms.heroImage || 'images/hero.png'}" style="width:90px; height:90px; object-fit:contain; border:1px solid var(--border-color); border-radius:6px; background:#FFF; padding:4px;">
                                    <div style="flex:1; display:flex; flex-direction:column; gap:8px;">
                                        <button type="button" class="btn btn-outline" id="cms-hero-img-btn" style="padding:8px 15px; font-size:0.75rem; font-weight:700; cursor:pointer;"><i class="fa-solid fa-upload"></i> Upload New Hero Image</button>
                                        <input type="file" id="cms-hero-file-input" accept="image/*" style="display:none;">
                                        <input type="text" id="cms-hero-img-url" class="form-control" value="${cms.heroImage || ''}" placeholder="Image URL or File Path" style="width:100%; padding:8px; border:1px solid var(--border-color); border-radius:4px; font-size:0.8rem;">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                <label for="cms-why-title" style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">"Why Choose Us" Section Heading</label>
                                <input type="text" id="cms-why-title" class="form-control" value="${cms.whyTitle || ''}" required style="width:100%; padding:10px; border:1px solid var(--border-color); border-radius:4px; font-weight:700;">
                            </div>

                            <div class="form-group">
                                <label style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">"Why Choose Us" Section Image</label>
                                <div style="display:flex; gap:15px; align-items:center;">
                                    <img id="cms-why-img-preview" src="${cms.whyImage || ''}" style="width:90px; height:90px; object-fit:cover; border:1px solid var(--border-color); border-radius:6px; background:#FFF; padding:4px;">
                                    <div style="flex:1; display:flex; flex-direction:column; gap:8px;">
                                        <button type="button" class="btn btn-outline" id="cms-why-img-btn" style="padding:8px 15px; font-size:0.75rem; font-weight:700; cursor:pointer;"><i class="fa-solid fa-upload"></i> Upload Image</button>
                                        <input type="file" id="cms-why-file-input" accept="image/*" style="display:none;">
                                        <input type="text" id="cms-why-img-url" class="form-control" value="${cms.whyImage || ''}" placeholder="Image URL" style="width:100%; padding:8px; border:1px solid var(--border-color); border-radius:4px; font-size:0.8rem;">
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- About Page Section CMS -->
                    <div style="border:1px solid var(--border-color); border-radius:6px; padding:20px; background:#FAFAFA;">
                        <h4 style="font-family:var(--font-heading); text-transform:uppercase; font-size:0.9rem; margin-bottom:15px; color:#000; display:flex; align-items:center; gap:8px;">
                            <i class="fa-solid fa-book-open" style="color:var(--text-muted);"></i> 2. About Us Page Content & Assets
                        </h4>

                        <div style="display:flex; flex-direction:column; gap:15px;">
                            <div class="form-group">
                                <label for="cms-about-title" style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">About Page Main Title</label>
                                <input type="text" id="cms-about-title" class="form-control" value="${cms.aboutTitle || ''}" required style="width:100%; padding:10px; border:1px solid var(--border-color); border-radius:4px; font-weight:700;">
                            </div>

                            <div class="form-group">
                                <label for="cms-about-para1" style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">Paragraph 1 (Welcome & Philosophy)</label>
                                <textarea id="cms-about-para1" class="form-control" rows="2" required style="width:100%; padding:10px; border:1px solid var(--border-color); border-radius:4px;">${cms.aboutPara1 || ''}</textarea>
                            </div>

                            <div class="form-group">
                                <label for="cms-about-para2" style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">Paragraph 2 (Story & Mission)</label>
                                <textarea id="cms-about-para2" class="form-control" rows="3" style="width:100%; padding:10px; border:1px solid var(--border-color); border-radius:4px;">${cms.aboutPara2 || ''}</textarea>
                            </div>

                            <div class="form-group">
                                <label for="cms-about-quote" style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">Highlighted Quote Box Text</label>
                                <textarea id="cms-about-quote" class="form-control" rows="2" style="width:100%; padding:10px; border:1px solid var(--border-color); border-radius:4px; font-style:italic;">${cms.aboutQuote || ''}</textarea>
                            </div>

                            <div class="form-group">
                                <label for="cms-about-cruelty" style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">Cruelty-Free / Sustainable Text</label>
                                <input type="text" id="cms-about-cruelty" class="form-control" value="${cms.aboutCrueltyText || ''}" style="width:100%; padding:10px; border:1px solid var(--border-color); border-radius:4px;">
                            </div>

                            <!-- About Image File/URL -->
                            <div class="form-group">
                                <label style="display:block; font-size:0.75rem; text-transform:uppercase; font-weight:700; margin-bottom:5px;">About Us Story Image</label>
                                <div style="display:flex; gap:15px; align-items:center;">
                                    <img id="cms-about-img-preview" src="${cms.aboutImage || ''}" style="width:90px; height:90px; object-fit:cover; border:1px solid var(--border-color); border-radius:6px; background:#FFF; padding:4px;">
                                    <div style="flex:1; display:flex; flex-direction:column; gap:8px;">
                                        <button type="button" class="btn btn-outline" id="cms-about-img-btn" style="padding:8px 15px; font-size:0.75rem; font-weight:700; cursor:pointer;"><i class="fa-solid fa-upload"></i> Upload Story Image</button>
                                        <input type="file" id="cms-about-file-input" accept="image/*" style="display:none;">
                                        <input type="text" id="cms-about-img-url" class="form-control" value="${cms.aboutImage || ''}" placeholder="Image URL" style="width:100%; padding:8px; border:1px solid var(--border-color); border-radius:4px; font-size:0.8rem;">
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <button type="submit" class="btn btn-chrome" style="height:48px; background:#000000; color:#FFFFFF; border:none; border-radius:4px; font-weight:700; cursor:pointer; width:fit-content; padding:0 30px; font-size:0.9rem;">
                        <i class="fa-solid fa-floppy-disk" style="margin-right:8px;"></i> Save All Content CMS Settings
                    </button>
                </form>
            </div>
        `;
                ...
                <div class="auth-card" style="max-width: 450px; width:100%; box-shadow: var(--shadow-lg); border: 1px solid var(--border-dark); background:#FFF; padding: 40px 30px; border-radius:8px;">
                    ...
                    <form id="admin-login-form">
                        <div class="form-group" style="margin-bottom:15px;">
                            ...
                            <input type="email" id="admin-email" class="form-control" placeholder="Enter Admin Email" required style="width:100%; padding:10px; border:1px solid var(--border-color); border-radius:4px;">
                        </div>
                ...
                <div class="admin-container" style="display: grid; grid-template-columns: 240px 1fr; min-height: calc(100vh - 160px); gap:30px;">
                    ...
                    <aside class="admin-sidebar" style="background: #FFFFFF; border: 1px solid var(--border-color); border-radius:8px; padding: 25px 15px; display: flex; flex-direction: column; gap: 20px; height:fit-content;">
                        ...
                        <nav style="display: flex; flex-direction: column; gap: 5px;">
                            ...
                            <a href="#" class="admin-nav-link ${activeTab === "cms" ? "active" : ""}" data-tab="cms" style="display: flex; align-items: center; gap: 10px; padding: 12px 15px; border-radius: 4px; text-decoration: none; color: var(--text-main); font-weight: 600; font-size: 0.85rem;"><i class="fa-solid fa-pen-to-square" style="width:20px;"></i> Content CMS</a>
                        </nav>
    ...
    const renderDashboardContent = () => {
        ...
        if (activeTab === "dashboard") {
            ...
            mainContent.innerHTML = renderCMSTab();
            bindCMSEvents();
        }
    ...
    const bindCMSEvents = () => {
        ...
        const bindFilePicker = (btnId, inputId, urlInputId, previewImgId) => {
        ...
        bindFilePicker("cms-hero-img-btn", "cms-hero-file-input", "cms-hero-img-url", "cms-hero-img-preview");
        bindFilePicker("cms-why-img-btn", "cms-why-file-input", "cms-why-img-url", "cms-why-img-preview");
        bindFilePicker("cms-about-img-btn", "cms-about-file-input", "cms-about-img-url", "cms-about-img-preview");
        ...
        form.onsubmit = (e) => {
            ...
            const data = {
                ...
                heroImage: document.getElementById("cms-hero-img-url").value.trim(),
                whyTitle: document.getElementById("cms-why-title").value.trim(),
                whyImage: document.getElementById("cms-why-img-url").value.trim(),
                aboutTitle: document.getElementById("cms-about-title").value.trim(),
                aboutPara1: document.getElementById("cms-about-para1").value.trim(),
                aboutPara2: document.getElementById("cms-about-para2").value.trim(),
                aboutQuote: document.getElementById("cms-about-quote").value.trim(),
                aboutCrueltyText: document.getElementById("cms-about-cruelty").value.trim(),
                aboutImage: document.getElementById("cms-about-img-url").value.trim()
            };

            Store.saveCMSContent(data);
            showToast("Content CMS Settings saved successfully!", "success");
            renderDashboardContent();
        };
    };
    ...
    const bindOrderEvents = () => {
        ...
        document.querySelectorAll(".view-order-details-btn").forEach(btn => {
            btn.addEventListener("click", () => {
                ...
                const customer = Store.getCustomers().find(c => c.email === o.customerEmail);
                const phoneNum = o.customerPhone || o.phone || (customer && customer.phone !== "N/A" ? customer.phone : null) || "N/A";
                ...
                content.innerHTML = `
                    <div style="margin-bottom:20px; font-size:0.88rem; line-height:1.6;">
                        ...
                        ${o.razorpayPaymentId ? `<p><strong>Razorpay Payment ID:</strong> <code style="color:#27ae60; font-family:monospace;">${o.razorpayPaymentId}</code></p>` : ''}
    ...
    const bindCustomerEvents = () => {
        document.querySelectorAll(".view-customer-details-btn").forEach(btn => {
            btn.addEventListener("click", () => {
                ...
                if (modalTitle) modalTitle.innerText = `Customer Activity: ${customer.name}`;

                // Liked products list
                const likedProducts = (customer.likedProductIds || [])
                    .map(id => Store.getProductById(id))
                    .filter(Boolean);

                const likedHtml = likedProducts.length > 0 ? likedProducts.map(p => `
                    <div style="display:flex; align-items:center; justify-content:space-between; padding:10px 12px; border:1px solid var(--border-color); border-radius:6px; margin-bottom:8px; background:#FAFAFA;">
                        <div style="display:flex; align-items:center; gap:12px;">
                            ...
                            <div>
                                ...
                                <span style="font-size:0.75rem; color:var(--text-muted);">${p.category} · ₹${p.price}</span>
                            </div>
                        </div>
                        <span style="font-size:0.75rem; color:#e74c3c; font-weight:700;"><i class="fa-solid fa-heart"></i> Liked</span>
                    </div>
                `).join("") : `<p style="font-size:0.85rem; color:var(--text-muted); font-style:italic;">No products liked yet.</p>`;

                // Orders history list
                const ordersHtml = (customer.orders || []).length > 0 ? customer.orders.map(ord => `
                    <div style="border:1px solid var(--border-color); border-radius:6px; padding:12px; margin-bottom:12px; background:#FFF;">
                        <div style="display:flex; justify-content:space-between; align-items:center; font-size:0.85rem; margin-bottom:8px; border-bottom:1px dashed var(--border-color); padding-bottom:6px;">
                        ...
                        <div style="font-size:0.8rem; color:var(--text-muted); margin-bottom:6px;">
                            <span>Date: ${ord.date}</span> · <span>Payment: <strong>${ord.paymentMethod || 'COD'}</strong></span>
                            ${ord.razorpayPaymentId ? ` · <span style="color:#27ae60;">RZP ID: ${ord.razorpayPaymentId}</span>` : ''}
                        </div>
                        <div style="font-size:0.8rem; margin-bottom:6px;">
                            <strong>Items (${ord.items.length}):</strong> ${ord.items.map(i => `${i.name} (x${i.qty})`).join(", ")}
                        </div>
                        <div style="text-align:right; font-weight:800; font-size:0.9rem; color:var(--text-main);">
                            Total Paid: ₹${ord.total}
                        </div>
                    </div>
                `).join("") : `<p style="font-size:0.85rem; color:var(--text-muted); font-style:italic;">No orders placed yet.</p>`;

                content.innerHTML = `
                    <div style="margin-bottom:20px; background:var(--bg-primary); padding:15px; border-radius:6px; font-size:0.85rem; line-height:1.6;">
                        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px;">
                            <div><strong>Name:</strong> ${customer.name}</div>
                            <div><strong>Email:</strong> ${customer.email}</div>
                            <div><strong>Phone:</strong> ${customer.phone}</div>
                            <div><strong>Total Orders:</strong> ${customer.ordersCount}</div>
                            <div><strong>Total Spent:</strong> ₹${customer.totalSpent}</div>
                            <div><strong>Liked Products:</strong> ${likedProducts.length}</div>
                        </div>
                    </div>

                    <div style="margin-bottom:25px;">
                        <h4 style="font-family:var(--font-heading); text-transform:uppercase; font-size:0.78rem; letter-spacing:0.05em; color:var(--text-muted); margin-bottom:10px;">
                            <i class="fa-solid fa-heart" style="color:#e74c3c;"></i> Liked Products (${likedProducts.length})
                        </h4>
                        ${likedHtml}
                    </div>
                    ...
                    <div>
                        <h4 style="font-family:var(--font-heading); text-transform:uppercase; font-size:0.78rem; letter-spacing:0.05em; color:var(--text-muted); margin-bottom:10px;">
                            <i class="fa-solid fa-boxes-packing"></i> Order History (${customer.orders.length})
                        </h4>
                        ${ordersHtml}
                    </div>
                `;
                ...
                closeBtn.onclick = () => {
                    ...
                    if (modalTitle) modalTitle.innerText = "Order Details";
                };
            });
        });
    };
    ...
    const bindPaymentEvents = () => {
        const ribbonForm = document.getElementById("ribbon-settings-form");
        if (ribbonForm) {
            ribbonForm.onsubmit = (e) => {
                e.preventDefault();
                const text = document.getElementById("ribbon-text-input").value.trim();
                const enabled = document.getElementById("ribbon-enabled-select").value === "true";
                const bgColor = document.getElementById("ribbon-bg-color").value;
                const textColor = document.getElementById("ribbon-text-color").value;
                const speed = parseInt(document.getElementById("ribbon-speed-select").value) || 25;

                Store.saveAnnouncementRibbon({ enabled, text, bgColor, textColor, speed });
                showToast("Announcement Ribbon Settings saved successfully!", "success");
                renderDashboardContent();
            };
        }
        ...
        if (rzpForm) {
            rzpForm.onsubmit = (e) => {
                ...
                const mode = document.getElementById("rzp-mode").value;
                const enabled = document.getElementById("rzp-enabled").value === "true";
                Store.saveRazorpaySettings(keyId, keySecret, enabled, mode);
                showToast("Razorpay Gateway Credentials saved successfully!", "success");
                renderDashboardContent();
            };
        }
