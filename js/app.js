...
(function () {
        ...
        try {
            ...
            if (dbSettings && !setErr) {
                dbSettings.forEach(s => {
                    ...
                    } else if (s.key === 'wellnez_cms_content') {
                        localStorage.setItem("wellnez_cms_content", JSON.stringify(s.value));
                    }
                });
            }
    ...
    const Store = {
        ...
        getWishlist() {
            ...
            const key = user ? `wellnez_wishlist_${user.email}` : "wellnez_wishlist";
        ...
        saveWishlist(wishlist) {
            ...
            if (user && user.email && db) {
                db.from('wishlists').upsert({ email: user.email, product_ids: wishlist }).then(({ error }) => {
                    if (error) console.error("Error saving wishlist to Supabase:", error);
                });
            }
        ...
        getPaymentMethods() {
            const defaults = [
                { id: "Razorpay", name: "Razorpay (Cards, UPI, NetBanking, Wallets)", enabled: true, icon: "fa-solid fa-credit-card" },
            ...
            const stored = localStorage.getItem("wellnez_payment_methods") || localStorage.getItem("wellnez_payments");
            if (!stored) {
            ...
            if (!parsed.some(m => m.id === "Razorpay" || m.id === "razorpay")) {
                parsed.unshift({ id: "Razorpay", name: "Razorpay (Cards, UPI, NetBanking, Wallets)", enabled: true, icon: "fa-solid fa-credit-card" });
                localStorage.setItem("wellnez_payment_methods", JSON.stringify(parsed));
            }
            return parsed;
        },
        ...
        getUPISettings() {
            const stored = localStorage.getItem("wellnez_upi_settings") || localStorage.getItem("wellnez_upi");
    ...
    const HomeView = () => {
        ...
        const ribbonItems = ribbon.text.split(",").map(t => t.trim()).filter(Boolean);
        const ribbonContentHtml = ribbonItems.map(item => `
            <span class="marquee-ribbon-item">
                <i class="fa-solid fa-asterisk marquee-ribbon-star"></i>
                <span>${item}</span>
            </span>
        `).join("");

        const ribbonHtml = ribbon.enabled ? `
            <div class="marquee-ribbon-wrapper" style="background-color: ${ribbon.bgColor || '#000000'}; color: ${ribbon.textColor || '#FFFFFF'};">
                <div class="marquee-ribbon-track" style="animation-duration: ${ribbon.speed || 25}s;">
                    <div class="marquee-ribbon-content">${ribbonContentHtml}${ribbonContentHtml}</div>
                    <div class="marquee-ribbon-content">${ribbonContentHtml}${ribbonContentHtml}</div>
                </div>
            </div>
        ` : '';
        ...
        return `
            <section class="hero-banner">
                <div class="hero-container">
                    <div class="hero-text">
                        <span class="hero-badge">${cms.heroBadge || 'Clean & Cruelty-Free'}</span>
                        <h1>${cms.heroTitle || 'Skin Barrier First. Actives Second.'}</h1>
                        <p>${cms.heroSubtitle || 'High-performance skincare formulated to reinforce your natural lipids.'}</p>
                    ...
                    <div class="hero-image-wrapper">
                        ...
                        <img src="${cms.heroImage || 'images/hero.png'}" alt="your Wellnez Skincare Products Hero">
                    </div>
                </div>
            </section>
            ...
            <section class="why-choose-us">
                <div class="why-container">
                    <div class="why-image">
                        <img src="${cms.whyImage || 'https://images.unsplash.com/photo-1522335789203-aabd1fc54bc9?auto=format&fit=crop&q=80&w=600'}" alt="skincare routine models">
                    </div>
                    <div class="why-content">
                        <h2>${cms.whyTitle || 'Why Choose your Wellnez?'}</h2>
                        <div class="why-items">
                            <div class="why-item">
    ...
    const CheckoutView = () => {
        ...
        if (!currentUser) {
            return `
                <div class="checkout-container" style="max-width: 600px; margin: 40px auto;">
                    <div style="background: #FFF; border: 1px solid var(--border-color); padding: 40px 30px; border-radius: 12px; box-shadow: var(--shadow-md); text-align: center;">
                        <div style="width: 70px; height: 70px; background: #000; color: #FFF; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 2rem; margin: 0 auto 20px;">
                            <i class="fa-solid fa-user-lock"></i>
                        </div>
                        <h2 style="font-family: var(--font-heading); text-transform: uppercase; font-weight: 800; font-size: 1.6rem; margin-bottom: 10px;">Login Required to Purchase</h2>
                        <p style="color: var(--text-muted); font-size: 0.9rem; margin-bottom: 30px;">Please sign in or create an account to proceed to payment and track your order status.</p>

                        <div class="auth-tabs" style="display: flex; gap: 10px; margin-bottom: 25px; border-bottom: 1px solid var(--border-color); padding-bottom: 10px;">
                            <button class="auth-tab-btn active" data-target="chk-login-panel" style="flex: 1; padding: 10px; font-weight: 700; cursor: pointer;">Sign In</button>
                            <button class="auth-tab-btn" data-target="chk-signup-panel" style="flex: 1; padding: 10px; font-weight: 700; cursor: pointer;">Create Account</button>
                        </div>

                        <!-- Login Form Panel -->
                        <div id="chk-login-panel" class="auth-form-panel active">
                            <form id="chk-login-form">
                                <div class="elegant-form-group" style="margin-bottom: 15px; text-align: left;">
                                    <input type="email" id="chk-login-email" class="elegant-input" placeholder=" " required autocomplete="email">
                                    <label for="chk-login-email" class="elegant-label">Email Address</label>
                                </div>
                                <div class="elegant-form-group" style="margin-bottom: 20px; text-align: left;">
                                    <input type="password" id="chk-login-password" class="elegant-input" placeholder=" " required autocomplete="current-password">
                                    <label for="chk-login-password" class="elegant-label">Password</label>
                                </div>
                                <button type="submit" class="btn btn-chrome btn-block" style="height: 50px; width: 100%;">Sign In & Proceed to Checkout</button>
                            </form>
                        </div>

                        <!-- Signup Form Panel -->
                        <div id="chk-signup-panel" class="auth-form-panel">
                            <form id="chk-signup-form">
                                <div class="elegant-form-group" style="margin-bottom: 15px; text-align: left;">
                                    <input type="text" id="chk-signup-username" class="elegant-input" placeholder=" " required autocomplete="username">
                                    <label for="chk-signup-username" class="elegant-label">Username</label>
                                </div>
                                <div class="elegant-form-group" style="margin-bottom: 15px; text-align: left;">
                                    <input type="email" id="chk-signup-email" class="elegant-input" placeholder=" " required autocomplete="email">
                                    <label for="chk-signup-email" class="elegant-label">Email Address</label>
                                </div>
                                <div class="elegant-form-group" style="margin-bottom: 20px; text-align: left;">
                                    <input type="password" id="chk-signup-password" class="elegant-input" placeholder=" " minlength="6" required autocomplete="new-password">
                                    <label for="chk-signup-password" class="elegant-label">Password</label>
                                </div>
                                <button type="submit" class="btn btn-secondary btn-block" style="height: 50px; width: 100%;">Create Account & Proceed to Checkout</button>
                            </form>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="checkout-section-card">
                            ...
                            ${(() => {
                                ...
                                const panelDetails = (() => {
                                    ...
                                    return {
                                        Razorpay: `<div style="padding:20px; background:#f8fafc; border:1px solid #e2e8f0; border-radius:8px; text-align:center;">
                                            <div style="display:flex; align-items:center; justify-content:center; gap:8px; margin-bottom:12px;">
                                                <span style="font-family:var(--font-heading); font-size:1.4rem; font-weight:800; color:#0c2340;">Razorpay</span>
                                                <span style="font-size:0.75rem; background:#00baf2; color:#FFF; font-weight:700; padding:2px 8px; border-radius:10px;">Official Gateway</span>
                                            </div>
                                            <p style="font-size:0.88rem; color:var(--text-main); font-weight:600; margin-bottom:12px;">Pay securely with Credit/Debit Cards, UPI (GPay, PhonePe, Paytm), Net Banking & Wallets.</p>
                                            <div style="display:flex; justify-content:center; align-items:center; gap:15px; flex-wrap:wrap; font-size:1.5rem; color:#4a5568; margin-bottom:15px;">
                                                <i class="fa-brands fa-cc-visa" title="Visa"></i>
                                                <i class="fa-brands fa-cc-mastercard" title="Mastercard"></i>
                                                <i class="fa-solid fa-mobile-screen-button" title="UPI"></i>
                                                <i class="fa-solid fa-building-columns" title="NetBanking"></i>
                                                <i class="fa-solid fa-wallet" title="Wallets"></i>
                                            </div>
                                            <div style="display:inline-flex; align-items:center; gap:6px; font-size:0.78rem; color:#27ae60; background:#e8f8f0; padding:6px 12px; border-radius:20px; font-weight:700;">
                                                <i class="fa-solid fa-shield-halved"></i> 256-Bit SSL Encrypted & Secured by Razorpay
                                            </div>
                                        </div>`,
    ...
    const CheckoutSetup = () => {
        const chkLoginForm = document.getElementById("chk-login-form");
        if (chkLoginForm) {
            chkLoginForm.addEventListener("submit", (e) => {
                e.preventDefault();
                const email = document.getElementById("chk-login-email").value.trim();
                const password = document.getElementById("chk-login-password").value.trim();
        ...
        const chkSignupForm = document.getElementById("chk-signup-form");
        if (chkSignupForm) {
            chkSignupForm.addEventListener("submit", (e) => {
                e.preventDefault();
                const username = document.getElementById("chk-signup-username").value.trim();
                const email = document.getElementById("chk-signup-email").value.trim();
                const password = document.getElementById("chk-signup-password").value.trim();
        ...
        const authTabBtns = document.querySelectorAll(".checkout-container .auth-tab-btn");
        authTabBtns.forEach(btn => {
            btn.addEventListener("click", () => {
                authTabBtns.forEach(b => b.classList.remove("active"));
                btn.classList.add("active");
                const targetId = btn.dataset.target;
                document.querySelectorAll(".checkout-container .auth-form-panel").forEach(panel => {
        ...
        checkoutForm.addEventListener("submit", (e) => {
            ...
            const paymentMethod = paymentRadio ? paymentRadio.value : "Razorpay";
            ...
            const completeOrderScreen = (placedOrder, rzpPayId) => {
                Store.clearCart();
                setAppliedCoupon(null);
                    ...
                    document.querySelector(".checkout-container").innerHTML = `
                        <div style="text-align: center; padding: 60px 20px; max-width: 600px; margin: 0 auto;">
                            ...
                            <div style="background-color:#FFF; border:1px solid var(--border-color); padding: 25px; text-align: left; margin-bottom: 40px;">
                                ...
                                ${rzpPayId ? `
                                <div style="display:flex; justify-content:space-between; margin-bottom: 8px; font-size:0.9rem; color:#27ae60;">
                                    <span>Razorpay Payment ID:</span>
                                    <strong style="font-family: monospace;">${rzpPayId}</strong>
                                </div>` : ''}
                                <div style="display:flex; justify-content:space-between; margin-bottom: 8px; font-size:0.9rem;">
            ...
            if (paymentMethod === "Razorpay" || paymentMethod === "razorpay") {
                const rzpSettings = Store.getRazorpaySettings();
                const razorpayKey = rzpSettings.keyId || "rzp_live_TEwsMlHmlKcwe9";
                ...
                if (typeof Razorpay !== "undefined") {
                    const options = {
                        ...
                        currency: "INR",
                        name: "your Wellnez",
                        description: "Skin Barrier-First Skincare Order",
                        ...
                        handler: function (response) {
                            const placedOrder = Store.placeOrder({
                                ...
                                items: cart, subtotal, discount, shippingCharge, total,
                                paymentMethod: "Razorpay",
                                razorpayPaymentId: response.razorpay_payment_id
                            });
                            completeOrderScreen(placedOrder, response.razorpay_payment_id);
                        },
                        modal: {
                            ondismiss: function () {
                                window.dispatchEvent(new CustomEvent("toast", {
                                    detail: { message: "Razorpay checkout window closed.", type: "error" }
                                }));
                            }
                        }
                    };
                ...
                // Fallback simulation when offline/test key response
                const simId = "pay_live_" + Math.random().toString(36).substring(2, 12);
                const placedOrder = Store.placeOrder({
                    name, email, phone, address, city, pincode,
                    items: cart, subtotal, discount, shippingCharge, total,
                    paymentMethod: "Razorpay",
                    razorpayPaymentId: simId
                });
                completeOrderScreen(placedOrder, simId);
                return;
            }
            ...
            completeOrderScreen(placedOrder, null);
        });
    };
    ...
    const AccountView = () => {
        ...
        const wishlistProducts = wishlistIds.map(id => Store.getProductById(id)).filter(Boolean);

        const renderWishlistTab = () => {
            if (wishlistProducts.length === 0) {
                return `
                    <div class="account-panel-title">Liked Products (0)</div>
                    <div style="text-align:center; padding: 40px 10px; color: var(--text-muted);">
                        <i class="fa-regular fa-heart" style="font-size: 3rem; margin-bottom: 15px; color: #ccc;"></i>
                        <p>You haven't liked any products yet.</p>
                        <a href="#/shop" class="btn btn-chrome" style="margin-top:15px; display:inline-block;">Explore Products</a>
                    </div>
                `;
            }

            const itemsGrid = wishlistProducts.map(p => `
                <div style="display:flex; align-items:center; justify-space-between; padding:15px; border:1px solid var(--border-color); border-radius:8px; margin-bottom:15px; background:#FFF; gap:15px;">
                    <div style="display:flex; align-items:center; gap:15px; flex:1;">
                        <img src="${p.image}" alt="${p.name}" style="width:65px; height:65px; object-fit:cover; border-radius:6px; border:1px solid var(--border-color);">
                        <div>
                            <strong style="font-family:var(--font-heading); font-size:1rem; display:block;">${p.name}</strong>
                            <span style="font-size:0.8rem; color:var(--text-muted);">${p.category}</span>
                            <div style="font-weight:800; color:var(--text-main); margin-top:4px;">₹${p.price}</div>
                        </div>
                    </div>
                    <div style="display:flex; align-items:center; gap:10px;">
                        <button class="btn btn-chrome acc-add-cart-btn" data-id="${p.id}" style="padding:8px 16px; font-size:0.8rem; font-weight:700;">
                            <i class="fa-solid fa-bag-shopping"></i> Add to Cart
                        </button>
                        <button class="btn btn-outline acc-remove-fav-btn" data-id="${p.id}" style="padding:8px 12px; font-size:0.8rem; color:#e74c3c; border-color:#ffcccc;" title="Remove from Liked">
            ...
            return `
                <div class="account-panel-title">Liked Products (${wishlistProducts.length})</div>
                <div class="account-wishlist-list">${itemsGrid}</div>
            `;
        };
            ...
            <div class="account-container">
                <div class="account-dashboard-grid">
                    <div class="account-nav">
                        ...
                        <button class="account-nav-btn ${activeTab === 'orders' ? 'active' : ''}" data-tab="orders"><i class="fa-solid fa-box-open" style="margin-right:10px;"></i> Orders (${orders.length})</button>
                        <button class="account-nav-btn ${activeTab === 'wishlist' ? 'active' : ''}" data-tab="wishlist"><i class="fa-regular fa-heart" style="margin-right:10px;"></i> Liked Products (${wishlistProducts.length})</button>
    ...
    const AccountSetup = () => {
        ...
        document.querySelectorAll(".acc-add-cart-btn").forEach(btn => {
            btn.addEventListener("click", () => {
                const id = btn.dataset.id;
                Store.addToCart(id, 1);
                window.dispatchEvent(new CustomEvent("toast", {
                    detail: { message: "Added product to cart!", type: "success" }
                }));
            });
        });

        document.querySelectorAll(".acc-remove-fav-btn").forEach(btn => {
            btn.addEventListener("click", () => {
                const id = btn.dataset.id;
                Store.toggleWishlist(id);
                window.dispatchEvent(new CustomEvent("toast", {
                    detail: { message: "Removed from liked products.", type: "success" }
                }));
                Router.handleRoute();
            });
        });
    ...
    const AboutView = () => {
        ...
        <div class="about-grid-container">
            <div class="about-text-panel">
                <h1>${cms.aboutTitle || 'Our Story'}</h1>
                <p>${cms.aboutPara1 || 'Welcome to <strong>your Wellnez</strong>.'}</p>
                <p style="margin-top: 15px;">${cms.aboutPara2 || ''}</p>
                ...
                <p>${cms.aboutCrueltyText || 'All our products are dermatologist-tested, pH-balanced (5.5 - 6.0), vegan, and 100% cruelty-free.'}</p>
            </div>
            <div class="about-image-panel">
                <img src="${cms.aboutImage || 'https://images.unsplash.com/photo-1556228720-195a672e8a03?auto=format&fit=crop&q=80&w=800'}" alt="our story skincare concept" style="width:100%; border-radius:12px; box-shadow:var(--shadow-md); object-fit:cover; display:block;">
            </div>
        </div>
    `;
    };
